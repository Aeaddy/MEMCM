...
$ArgList += @("`"C:\Program Files\SCCMConsoleExtensions\ClearCacheCol.ps1`"")
$ArgList += @("`"$strColID`"")
$ArgList += @("`"$strColName`"")
...
$OnLoadForm_StateCorrection=
{
	...
	$msg = "Do you really want to clear the cache on all computers in $strColName"
	...
	if ($PopupAnswer -eq 1){
		...
		Get-WmiObject -Query $strQuery -Namespace $strNamespace -ComputerName $strServer | ForEach-Object {
			...
			$CompName = $_.Name
			Start-Job -Name $Jobs -ArgumentList $CompName -ScriptBlock {
				$CompName = $null
				$CompName = $args[0]
				...
				If (test-connection -computername $CompName -count 1 -quiet){
					...
					Copy-Item "C:\Program Files\SCCMConsoleExtensions\clearClientCache.vbs" "\\$CompName\ADMIN$\" -Force
					...
					if ($checkcache -ne $null){$strErrorMsg = "$CompName \/WMI Error\/"}
			...
			while (((get-job | where-object { $_.Name -like "$Jobs" -and $_.State -eq "Running" }) | measure).Count -gt 10){
					[System.Windows.Forms.Application]::DoEvents()
