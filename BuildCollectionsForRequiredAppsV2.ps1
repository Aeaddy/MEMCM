...
Purpose: The purpose of the script is to automatically deploy Applications for use with Standard Software Deployments.  This script will identify applications with a specified Category, and
...
#Set these values to: 
#The name of the SCCM folder to store the Software device collections.
$SNDIR = 'BOK Automatic-Install Software'
...
Param ($AppName)
    Get-CMDeviceCollection -Name "*$AppName*_R-Install"
}

Function checkForUninstallCollection{
Param ($AppName)
    Get-CMDeviceCollection -Name "*$AppName*_R-Uninstall"
}
...
Param ($IAppToDeploy, $INewCollName)
    ...
    New-CMApplicationDeployment -InputObject $CMAPPI -DeployAction Install -DeployPurpose $DeploymentAvailability -Collection $CMCOLLI
}
...
$LOGFILE = "CreateAppCollectionsAndDeploy_" + $FORMATTEDDATE + ".log"
...
write-output "$UFORMATTEDDATE - $($SNAPP.LocalizedDisplayName) : Begin Collection, Distribution and Deployment Check" | out-file $LOGPATH -Append -Force -NoClobber
    ...
    if ($ContentExists -eq $true) { 
        ...
        if (!$iAPPDEPCHECK) { 
            $UFORMATTEDDATE = get-date
                write-output "$UFORMATTEDDATE - Warning! A deployment for $ICOLLNAME does not exist. Attempting to deploy the installation in a $DeploymentAvailability state." | out-file $LOGPATH -Append -Force -NoClobber
