...
$NEWLOGFILE ="$UTEMP\Drivers\$COMPMODEL.log"
$DriverPackageName = $COMPMODEL
$DriverPackageSource="$PKGSHARENAME\$COMPMODEL"
$DriverSource="$SHARENAME\$COMPMODEL"
...
Export-WindowsDriver -Destination "$DRIVERPATH\$((Get-WmiObject -Class win32_computersystem).Model)" -Online | out-file $LOGFILE
