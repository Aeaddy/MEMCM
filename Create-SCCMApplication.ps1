<#
.SYNOPSIS
    This script is used with System Center Configuration Manager.
    The script can automatically create an Application, MSI deployment type, AD Group, User/Device Collection based on the AD group and a Deployment.
    ...
    NOTE: The script has only been fully tested and verified with PowerShell v4.0, on Windows Server 2012 R2 with SCCM 2012 R2 CU3.
        
.DESCRIPTION
    Use this script to create a new Application in SCCM 2012. The minimum required input is a name for the Application. This will simply create an Application with that name.
    ...
    The GUI is dynamic and will only enable options when a dependent options has also been selected. An MST file for example, can only be selected if an MSI file has been selected first.
    Creating a deployment is only available if creating a collection and content is distributed, etc.
...
.LINK
    ...
    PowerShell App Deployment Toolkit: https://psappdeploytoolkit.codeplex.com/
...
.EXAMPLE
    Run the scrip and create a collection folder called "MyCollectionFolder". Create a collection and move it to that folder.

        .\Create-Application.ps1 -CollectionRootFolderName "MyCollectionFolder"
...
Param(
    ...
    # Enter the name of your distribution point group here
    $DPGroup = "Campus",

    # Enter the Netbios name of your domain here
    $DomainNetbiosName,
    
    # Paths to where the AD groups will be created (can be different paths for users and devices)
    $UserOUPath = "OU=Application Groups,OU=Groups,OU=LABB,DC=labb,DC=local",
    $DeviceOUPath = "OU=Application Groups,OU=Groups,OU=LABB,DC=labb,DC=local",
    
    # Prefix to the name of the AD groups that will be created. By default the group will be called "<prefix><Name of Application>" but can be manually edited in the GUI.
    $ADGroupNamePrefix = "",
    ...
    # By default, if an Application folder is created it will be named by the publisher/manufacturer of the application (i.e. Microsoft or Adobe). If you wish a different name, specify it here. If you leave this empty, and also leaves the publisher/manufacturer emtpy, then no folder will be created.
    $ApplicationFolderName = "",

    # If this is set to $true an Collection folder will be created and the Collection will be moved to that folder. If set to $false, no folder will be created and the Collection will be placed in the root.
    [bool]$CreateCollectionFolder = $true,

    # Select a name for the collection folder. If you leave this blank, no folder will be created and the collection will be created in the root.
    $CollectionRootFolderName = "Software Installs",
   
    # Name of device- and user collections used as limiting collections
    $DeviceLimitingCollection = "All Desktop and Server Clients",
    $UserLimitingCollection = "All Users and User Groups",

    # If a deployment type is created (if an MSI-package is selected), this flag determines if installation will run as 32-bit process on 64-bit systems.
...
#If no site code was passed in then try and grab it from the local machine.
If (-Not ($SiteCode)) {$SiteCode=Get-Site}

#If no domain was passed try and grab it from the local machine.
If (-Not ($DomainNetbiosName)) {$DomainNetbiosName=[System.Security.Principal.WindowsIdentity]::GetCurrent().Name.Split('\\')[0]}
...
function Generate-GUI
{
    ...
    $Form1.ClientSize = New-Object System.Drawing.Size(689, 456)
    ...
    $ProgressBar1.Location = New-Object System.Drawing.Point(301, 421)
    ...
    $ButtonCreate.Location = New-Object System.Drawing.Point(582, 365)
    ...
    $ButtonPADT.Location = New-Object System.Drawing.Point(582, 322)
    ...
    $LabelPurpose.Location = New-Object System.Drawing.Point(124, 316)
    ...
    $LabelTarget.Location = New-Object System.Drawing.Point(12, 316)
    ...
    $LabelAuthor.Location = New-Object System.Drawing.Point(12, 415)
    $LabelAuthor.Size = New-Object System.Drawing.Size(140, 15)
    $LabelAuthor.Font = New-Object System.Drawing.Font("Microsoft Sans Serif",8.25,0,3,0)
    $LabelAuthor.Text = "Author: Joachim Bryttmar"
    #~~< LabelBlog >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    $LabelBlog = New-Object System.Windows.Forms.Label
    $LabelBlog.Location = New-Object System.Drawing.Point(12, 430)
    $LabelBlog.Size = New-Object System.Drawing.Size(40, 23)
    $LabelBlog.Font = New-Object System.Drawing.Font("Microsoft Sans Serif",8.25,0,3,0)
    #$LabelBlog.Text = "Blog:"
    #~~< LinkLabelBlog >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    $LinkLabelBlog = New-Object System.Windows.Forms.LinkLabel
    $LinkLabelBlog.Location = New-Object System.Drawing.Point(50, 430)
    $LinkLabelBlog.Size = New-Object System.Drawing.Size(140,23)
    $LinkLabelBlog.Font = New-Object System.Drawing.Font("Microsoft Sans Serif",8.25,0,3,0)
    #$LinkLabelBlog.Text = "infogeek.se"
    #$LinkLabelBlog.add_click({start("http://www.infogeek.se")})
    #~~< PanelPurpose >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    $PanelPurpose = New-Object System.Windows.Forms.Panel
    $PanelPurpose.Location = New-Object System.Drawing.Point(124, 327)
    ...
    $PanelTarget.Location = New-Object System.Drawing.Point(12, 327)
    ...
    $CheckBoxCreateDeployment.Location = New-Object System.Drawing.Point(407, 370)
    ...
    $CheckBoxDistributeContent.Location = New-Object System.Drawing.Point(406, 340)
    ...
    #~~< CheckBoxUseADGroup >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    $CheckBoxUseADGroup = New-Object System.Windows.Forms.CheckBox
    $CheckBoxUseADGroup.Checked = $true
    $CheckBoxUseADGroup.CheckState = [System.Windows.Forms.CheckState]::Checked
    $CheckBoxUseADGroup.Location = New-Object System.Drawing.Point(277, 370)
    $CheckBoxUseADGroup.RightToLeft = [System.Windows.Forms.RightToLeft]::Yes
    $CheckBoxUseADGroup.Size = New-Object System.Drawing.Size(123, 24)
    $CheckBoxUseADGroup.TabIndex = 20
    $CheckBoxUseADGroup.Text = "Use AD Group"
    $CheckBoxUseADGroup.UseVisualStyleBackColor = $true
    $CheckBoxUseADGroup.add_CheckedChanged({CheckBoxUseADGroupChanged})
    ...
    $CheckBoxCreateADGroup.Location = New-Object System.Drawing.Point(277, 400)
    ...
    $CheckBoxCreateCollection.Location = New-Object System.Drawing.Point(277, 340)
    ...
    $ButtonMSI.Location = New-Object System.Drawing.Point(582, 70)
    $ButtonMSI.Size = New-Object System.Drawing.Size(75, 23)
    $ButtonMSI.TabIndex = 0
    ...
    $LabelScriptVersion.Location = New-Object System.Drawing.Point(600, 424)
    ...
    $LabelADGroup.Location = New-Object System.Drawing.Point(12, 284)
    $LabelADGroup.Size = New-Object System.Drawing.Size(109, 13)
    ...
    $LabelSourcePath.Location = New-Object System.Drawing.Point(12, 258)
    ...
    $LabelUnInstallProgram.Location = New-Object System.Drawing.Point(12, 232)
    ...
    $LabelInstallProgram.Location = New-Object System.Drawing.Point(12, 206)
    ...
    $LabelVersion.Location = New-Object System.Drawing.Point(12, 180)
    ...
    $LabelPublisher.Location = New-Object System.Drawing.Point(12, 154)
    ...
    $LabelAppName.Location = New-Object System.Drawing.Point(12, 128)
    ...
    $TextBoxADGroup.Location = New-Object System.Drawing.Point(139, 281)
    $TextBoxADGroup.Size = New-Object System.Drawing.Size(428, 20)
    $TextBoxADGroup.TabIndex = 0
    $TextBoxADGroup.Text = ""
    #~~< TextBoxSourcePath >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    $TextBoxSourcePath = New-Object System.Windows.Forms.TextBox
    $TextBoxSourcePath.Location = New-Object System.Drawing.Point(139, 255)
    ...
    $TextBoxPublisher.Location = New-Object System.Drawing.Point(139, 151)
    $TextBoxPublisher.Size = New-Object System.Drawing.Size(428, 20)
    $TextBoxPublisher.TabIndex = 0
    $TextBoxPublisher.Text = ""
    #~~< TextBoxAppName >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    $TextBoxAppName = New-Object System.Windows.Forms.TextBox
    $TextBoxAppName.Location = New-Object System.Drawing.Point(139, 125)
    ...
    $Form1.Controls.Add($CheckBoxUseADGroup)
    ...
    #~~< ErrorProviderApplicationName >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    $ErrorProviderApplicationName = New-Object System.Windows.Forms.ErrorProvider($components)
    $ErrorProviderApplicationName.BlinkStyle = [System.Windows.Forms.ErrorBlinkStyle]::NeverBlink
    $ErrorProviderApplicationName.ContainerControl = $Form1
...
Function Load-Form
{
    $ErrorProviderApplicationName.SetError($TextBoxAppName, "Please enter a name for the application")
    ...
    $CheckBoxUseADGroup.Checked = $false
...
Function AppName-Changed
{
    ...
    {
        $ErrorProviderApplicationName.Clear()
...
Function ButtonPADTClick
{
    ...
    $TextBoxInstallProgram.Text = "ServiceUI.exe Deploy-Application.exe"
    $TextBoxUnInstallProgram.Text = "ServiceUI.exe Deploy-Application.exe Uninstall"

    # Disable the MST browse button
    $ButtonMST.Enabled = $False
    $TextBoxMSTFile.Enabled = $False

    # Set flag to run installation as 32-bit process on 64-bit systems. This is necessary when using ServiceUI.exe
    $global:RunInstallAs32Bit = $true
}

# Function to control actions when clicking on button 'Create'
Function ButtonCreateClick
{

    # Remember the current location and go to the SCCM site location.
    $CurrentLocation = Get-Location
    Set-Location ($SiteCode + ":")
    ...
    # Check if the application already exists, if not create it
    If ((Check-ApplicationExist $ApplicationName) -ne $true)
    {
        # Create the application
        New-CMApplication -Name $ApplicationName -Publisher $Publisher -AutoInstall $true -SoftwareVersion $ApplicationVersion
        ...
        If ($CreateApplicationFolder -and $FolderName -ne "")
        {
            ...
            If (-not (Test-Path $ApplicationFolderPath))
            {
                ...
                Write-Host "Created application folder $FolderName"
                $ProgressBar1.PerformStep()
            }
            ...
            Write-Host "Moved application $ApplicationName to folder $FolderName"
            $ProgressBar1.PerformStep()
        }
        ...
        $ADGroupName = $TextBoxADGroup.Text
         # Set path to OU depending on selected target
         If ($CheckBoxUseADGroup.Checked){
            If ($RadioButtonUser.Checked) { $OUPath = $UserOUPath }
            If ($RadioButtonDevice.Checked) { $OUPath = $DeviceOUPath }
        }

        If ($CheckBoxCreateDGroup.Checked)
        {
            New-ADGroup -Name $ADGroupName -Path $OUPath -Description "Members of this group will be targeted for deployment of $ApplicationName in SCCM." -GroupScope Global
            Write-Host "Created AD group $ADGroupName in $OUPath"
            $ProgressBar1.PerformStep()
        }
        ...
        If ($CheckBoxCreateCollection.Checked)
        {
            ...
            {
                $OUPath = $UserOUPath
                $CollectionFolderPath = $SiteCode + ":" + "\UserCollection\$CollectionRootFolderName"
            }
            If ($RadioButtonDevice.Checked)
            {
                $OUPath = $DeviceOUPath
                If ($CreateCollectionFolder -and $Publisher -ne ""){
                    $CollectionFolderPath = $SiteCode + ":" + "\DeviceCollection\$CollectionRootFolderName\$Publisher"
                    }
                Else {
                    $CollectionFolderPath = $SiteCode + ":" + "\DeviceCollection\$CollectionRootFolderName"
                    }
            }

            # Create the user/device-collection folder, if one has been specified in the parameters and if it does not exist
            If ($CollectionRootFolderName -ne "")
            {
               If (-not (Test-Path $CollectionFolderPath))
               {
                   New-Item $CollectionFolderPath
                   Write-Host "Created collection folder $CollectionRootFolderName"
                   $ProgressBar1.PerformStep()
             }
            }

            $Schedule = New-CMSchedule -Start (Random-StartTime) –RecurInterval Days –RecurCount 1
            $CollectionName = "$ApplicationName - Install"
            If ($RadioButtonDevice.Checked)
            {
                #Get the collection if it already exists
                $DeploymentCollection = Get-CMDeviceCollection -Name $CollectionName

                If ($DeploymentCollection) {
                    Write-Host "Retrieved existing device collection $CollectionName"
                }
                Else {
                    $DeploymentCollection = New-CMDeviceCollection -Name $CollectionName -LimitingCollectionName $DeviceLimitingCollection -RefreshType Both -RefreshSchedule $Schedule
                    Write-Host "Created device collection $CollectionName"
                }
                
                $ProgressBar1.PerformStep()
                If ($CheckBoxUseADGroup.Checked){
                    Add-CMDeviceCollectionQueryMembershipRule -Collection $DeploymentCollection -QueryExpression "select *  from  SMS_R_System where SMS_R_System.SystemGroupName = ""$DomainNetbiosName\\$ADGroupName""" -RuleName "Members of AD group $ADGroupName"
                    }
                # Check if a collection folder name has been specified, then move the collection there
                If ($CollectionRootFolderName -ne "")
                {
                    Move-CMObject -FolderPath $CollectionFolderPath -InputObject $DeploymentCollection
                    Write-Host "Moved collection $CollectionName to folder $CollectionRootFolderName"
                    $ProgressBar1.PerformStep()
                }
            }
            ...
            If ($RadioButtonUser.Checked)
            {
                $DeploymentCollection = New-CMUserCollection -Name $CollectionName -LimitingCollectionName $UserLimitingCollection -RefreshType Both -RefreshSchedule $Schedule
                Write-Host "Created user collection $CollectionName"
                $ProgressBar1.PerformStep()
                If ($CheckBoxUseADGroup.Checked){
                    Add-CMUserCollectionQueryMembershipRule -Collection $DeploymentCollection -QueryExpression "select * from SMS_R_User where SMS_R_User.SecurityGroupName = ""$DomainNetbiosName\\$ADGroupName""" -RuleName "Members of AD group $ADGroupName"
                }
                # Check if a collection folder name has been specified, then move the collection there
                If ($CollectionRootFolderName -ne "")
                {
        ...
        # If an MSI package is selected and a source path provided then create a deployment type, add the application to distribution point group, and deploy the application
        $MSIFilePath = $TextBoxMSIPackage.Text
        If (($MSIFilePath.Length -gt 0) -and ($TextBoxSourcePath.Text.Length -gt 0))
        {
            Add-CMMsiDeploymentType -ApplicationName $ApplicationName -ContentLocation $MSIFilePath -Force -UserInteractionMode Hidden
            Write-Host "Created deployment type based on $MSIFilePath for $ApplicationName"
            $ProgressBar1.PerformStep()
        
            # Change installation and uninstallation programs, and content source path, if specified
            ...
            If ($TextBoxInstallProgram.Text.Length -gt 0)
            {
                ...
                Write-Host "Installation program set to: $InstallationProgram"
                $ProgressBar1.PerformStep()
            }
            If ($TextBoxUnInstallProgram.Text.Length -gt 0)
            {
                ...
                Write-Host "Uninstallation program set to: $UnInstallationProgram"
                $ProgressBar1.PerformStep()
            }           
            ...
            If ($CheckBoxDistributeContent.Checked)
            {
                ...
                Write-Host "Distributed content for $ApplicationName to $DPGroup"
                $ProgressBar1.PerformStep()
            }
            ...
            {
                Start-CMApplicationDeployment -CollectionName $DeploymentCollection.Name -Name $ApplicationName -DeployPurpose $DeployPurpose
                Write-Host "Created $DeployPurpose deployment for $ApplicationName to collection $CollectionName"
                $ProgressBar1.PerformStep()
            }
        }
    ...
    Else
    {
        $ErrorProviderApplicationName.SetError($TextBoxAppName, "An application called $ApplicationName already exists. Please check the name and try again.")        
        $ProgressBar1.Visible = $false
    }
...
{
    # If collection is not selected to be created, then clear selection to create deployment
    If ($CheckboxCreateCollection.Checked)
    {
        $CheckBoxCreateDeployment.Checked = $true
        $CheckBoxUseADGroup.Enabled = $true
    }
    Else
    {
        $CheckBoxCreateDeployment.Checked = $false
        $CheckBoxUseADGroup.Enabled = $false
    }
}
...
{
    # If deployment is selected to be created, then also create collection and distribute the content
    If ($CheckboxCreateDeployment.Checked)
    {
        $CheckBoxCreateCollection.Checked = $true
        $CheckBoxDistributeContent.Checked = $true
...
Function CheckBoxUseADGroupChanged
{
    # If the checkbox for creating AD group is unchecked, then clear the textbox with the AD group name and disable it
    If ($CheckBoxUseADGroup.Checked -eq $false)
    {
        $TextBoxADGroup.Enabled = $false
...
Function OpenMSIFile    
{
    ...
    [string]$MSIFilePath = $OpenFileDialogMSI.FileName
    ...
    # Search the source path to see if a single MST exists and add select it if found.
    If (Test-Path $SourcePath){
        $MSTs=(Get-ChildItem "\\w0982dappv0601.dus.meijer.com\Sources\Software\Adobe\Reader\11.0.07\01" | Where {$_.extension -eq ".mst"})
        If ($MSTs.Count -eq 1) {
                $TextBoxMSTFile.Text = $MSTs[0].FullName
            }
...
Function OpenMSTFile
{
    ...
    # Populate text box
    $TextBoxMSTFile.Text = $MSTFileName
